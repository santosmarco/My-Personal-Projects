...
def pull_coins_data(convert='USD'):
    ...
    url = ('https://api.coinmarketcap.com/v1/ticker/'
           + '?convert={}&limit=0'.format(convert))
...
def pull_global_data(convert='USD'):
    convert = convert.upper().strip()
    url = 'https://api.coinmarketcap.com/v1/global/?convert={}'.format(convert)
    raw_data = requests.get(url)
    try:
        raw_data.raise_for_status()
    except Exception:
...
def pull_historical_data(coin, convert='USD'):
    ...
    url = ('https://min-api.cryptocompare.com/data/'
           + 'histoday?fsym={coin}&tsym={}'.format(convert)
...
def prettify(data, convert):
    ...
    for coin in data:
        ...
        data[coin]['price'] = data[coin]['price_{}'.format(convert)]
        data[coin]['market_cap'] = data[coin]['market_cap_{}'.format(convert)]
        data[coin]['24h_volume'] = data[coin]['24h_volume_{}'.format(convert)]
        del data[coin]['price_{}'.format(convert)]
        del data[coin]['market_cap_{}'.format(convert)]
        del data[coin]['24h_volume_{}'.format(convert)]
    for coin in data:
        del data[coin]['id']
...
def get_global_data(convert='USD'):
    ...
    global_data['market_cap'] = global_data['total_market_cap_{}'.format(convert)]
    global_data['24h_volume'] = global_data['total_24h_volume_{}'.format(convert)]
